...
class Positloss(nn.Module):
    def __init__(self, reduction="none"):
        super(Positloss, self).__init__()
        self.reduction = reduction
    ...
    def forward(self, pred, target):
        ...
        loss = torch.norm(target - pred, dim=1) / torch.norm(target, dim=1)
...
class Orientloss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        loss = se3lib.compute_geodesic_distance_from_two_matrices(gt_r, est_r)
...
class YOLOLoss(nn.Module):
    def __init__(self, num_classes, fp16, strides=[8, 16, 32]):
        ...
        self.posit_loss = Positloss(reduction="none")
        self.orient_loss = Orientloss(reduction="none")
        self.grids = [torch.zeros(1)] * len(strides)
        self.fp16 = fp16

    def forward(self, inputs, labels=None, poses=None):
        outputs = []
        x_shifts = []
        y_shifts = []
        expanded_strides = []
        ...
        # -----------------------------------------------#
        # inputs    [[batch_size, num_classes + 5 + 12, 20, 20]
        #            [batch_size, num_classes + 5 + 12, 40, 40]
        #            [batch_size, num_classes + 5 + 12, 80, 80]]
        # outputs   [[batch_size, 400, num_classes + 5 + 12]
        #            [batch_size, 1600, num_classes + 5 + 12]
        #            [batch_size, 6400, num_classes + 5 + 12]]
        ...
        return self.get_losses(x_shifts, y_shifts, expanded_strides, labels, poses, torch.cat(outputs, 1))
    ...
    def get_output_and_grid(self, output, k, stride):
        ...
        # output[..., 7:9] = torch.exp(output[..., 2:4]) * stride
        return output, grid

    def get_losses(self, x_shifts, y_shifts, expanded_strides, labels, poses, outputs):
        # -----------------------------------------------#
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            if num_gt == 0:
                ...
                gt_loc_per_image = poses[batch_idx][..., :3].type_as(outputs)
                gt_rot_per_image = poses[batch_idx][..., 3:].type_as(outputs)
                ...
                loc_target = gt_loc_per_image[matched_gt_inds]
                rot_target = gt_rot_per_image[matched_gt_inds]
        ...
        loss_loc = (self.posit_loss(loc_preds.view(-1, 3)[fg_masks], loc_targets)).sum()
